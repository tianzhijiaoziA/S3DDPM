...
from complexUnet import Complex_relu as nn_SiLU
from complexUnet import ComplexConv2d as nn_Conv2d
from complexUnet import ComplexConv2d_init as nn_Conv2d_init
from complexUnet import Complex_maxpooling2d as nn_AvgPool2d
from complexUnet import ComplexBatchNorm2d

#不确定组装块需不需要
from complexUnet import complex_conv_block
from torch.nn import SiLU, Linear
...
class Downsample(nn.Module):
    def __init__(self, in_channels, use_conv, out_channels=None):
        ...
        if use_conv:
            # downsamples by 1/2
            self.downsample = nn_Conv2d(in_channels, out_channels, 3, stride=2, padding=1)
        else:
            assert in_channels == out_channels
            #这个复数POOLing需要后面加上卷积 复数的不会改
...
class Upsample(nn.Module):
    def __init__(self, in_channels, use_conv, out_channels=None):
        ...
        if use_conv:
            self.conv = nn_Conv2d(in_channels, out_channels, 3, padding=1)
...
class ResBlock(TimestepBlock):
    def __init__(
        ...
        self.in_layers = nn.Sequential(
                ComplexBatchNorm2d(in_channels),
                SiLU(),
                nn_Conv2d(in_channels, out_channels, 3, padding=1)
                )
        ...
        self.embed_layers = nn.Sequential(
                SiLU(),
                Linear(time_embed_dim, out_channels)
                )
        self.out_layers = nn.Sequential(
                ComplexBatchNorm2d(out_channels),
                SiLU(),
                nn.Dropout(p=dropout),
                zero_module(nn_Conv2d(out_channels, out_channels, 3, padding=1))
                )
        if out_channels == in_channels:
            self.skip_connection = nn.Identity()
        elif use_conv:
            self.skip_connection = nn_Conv2d(in_channels, out_channels, 3, padding=1)
        else:
            self.skip_connection = nn_Conv2d(in_channels, out_channels, 1)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.down = nn.ModuleList(
                [TimestepEmbedSequential(nn_Conv2d_init(self.in_channels, base_channels, kernel_size=3, stride=1, padding=1))]
                )
        channels = [ch]
        ds = 1
        ...
        self.out = nn.Sequential(
                ...
                SiLU(),
                zero_module(nn_Conv2d(base_channels * channel_mults[0], self.out_channels, 3, padding=1))
                )

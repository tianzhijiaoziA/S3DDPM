...
from OCT_dataloader import load_train_data, GetCT, GetCTMAT, load_test_data
...
def train(training_dataset_loader, testing_dataset_loader, args, resume):
    ...
    for epoch in tqdm_epoch:
        ...
        for i, data in enumerate(training_dataset_loader):
            ...
            optimiser.zero_grad()
            print('Epoch: %d.:%d Loss value: %.4f' % (epoch, i, loss))
            loss.backward()
...
def main():
    ...
    if args["dataset"].lower() == "cifar":
        ...
        training_dataset_loader = load_train_data(batch_size=2)
        testing_dataset_loader = load_test_data(batch_size=2)
        # training_dataset_loader = dataset.init_dataset_loader(training_dataset, args)
        # testing_dataset_loader = dataset.init_dataset_loader(testing_dataset, args)
    ...
    if False:
        if resume == 1:
            ...
            ##resume模型的
            print("resume")
            file_dir = f'./model/diff-params-ARGS={args["arg_num"]}/checkpoint/diff_epoch=50.pt'
            loaded_model = torch.load(file_dir, map_location=device)

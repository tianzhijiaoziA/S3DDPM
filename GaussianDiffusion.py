...
save_path='/data/wangyy/AnoDDPM/SS_DDPM_complex/diffusion-training-images/ARGS=10/'
...
class GaussianDiffusionModel:
    ...
    def sample_p(self, model, x_t, t, denoise_fn="gauss"):
        ...
        #sample = out["mean"] + nonzero_mask * torch.exp(0.5 * out["log_variance"]) * noise       #exp()里边越大，噪声越小
        return {"sample": out["mean"], "pred_x_0": out["pred_x_0"]}

    def forward_backward(
            self, model, x, see_whole_sequence="half", t_distance=None, denoise_fn="simplex",
            ):
        ...
        #         seq.append(x.cpu().detach())
        # vol_t_mean = np.zeros([1,int(t_distance),1024,1024],dtype=np.float32)
        # vol_t_mean=torch.from_numpy(vol_t_mean).to(x.device)
        ...
        for t in range(int(t_distance) - 1, -1, -1):
            ...
            with torch.no_grad():
                ...
                print("sample_p当前的采样步长",t)
            # if see_whole_sequence:
            #     seq.append(x.cpu().detach())
            # if t<499:
            #     vol_t_mean[:,t,:,:] = out["sample"]
            aaa=1#1表示的是看射线光子的图像，进行过转换的
            if aaa==1:
                if (t<480) and (t%5)==0:
                    plt.figure(figsize=(6,12))
                    plt.subplot(1,3,1),plt.imshow(-5054*np.exp(ori.squeeze().cpu().detach().numpy()),cmap='gray'),plt.axis('off'),plt.title('noisy')
                    plt.subplot(1,3,2),plt.imshow(-5054*np.exp(x.squeeze().cpu().detach().numpy()),cmap='gray'),plt.axis('off'),plt.title('sample_mean')
                    #plt.subplot(1,4,3),plt.imshow(-5054*np.exp(out["pred_x_0"].squeeze().cpu().detach().numpy()),cmap='gray'),plt.axis('off'),plt.title('sample_pred')
                    #plt.subplot(1,3,3),plt.imshow(-5054*np.exp(torch.mean(vol_t_mean,1).squeeze().cpu().detach().numpy()),cmap='gray'),plt.axis('off'),plt.title('vol_t_mean')
                    # plt.subplot(1,4,4),plt.imshow(torch.mean(vol_t_pred,1).squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('vol_t_pred')
                    plt.show()
                    #io.savemat(save_path+str(t)+'DRjizhu.mat',{'vol_t_mean': torch.mean(vol_t_mean,1).squeeze().cpu().detach().numpy(),'sample_mean': x.squeeze().cpu().detach().numpy(),'input': ori.squeeze().cpu().detach().numpy()})

            else:
                if  (t<470) and (t%2)==0:
                    plt.figure(figsize=(6,6))
                    plt.subplot(1,4,1),plt.imshow(ori.squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('noisy')
                    plt.subplot(1,4,2),plt.imshow(x.squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('sample_mean')
                    plt.subplot(1,4,3),plt.imshow(out["pred_x_0"].squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('sample_pred')
                    plt.subplot(1,4,4),plt.imshow(torch.mean(vol_t_mean,1).squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('vol_t_mean')
                    #plt.subplot(1,4,4),plt.imshow(torch.mean(vol_t_pred,1).squeeze().cpu().detach().numpy(),cmap='gray'),plt.axis('off'),plt.title('vol_t_pred')
                    plt.show()
                    io.savemat(save_path+str(t)+'BRAIN.mat',{'vol_t_mean': torch.mean(vol_t_mean,1).squeeze().cpu().detach().numpy(),'sample_mean': x.squeeze().cpu().detach().numpy(),'input': ori.squeeze().cpu().detach().numpy()})
        return x.detach()
